...
{
  ZoneScopedN("EncodingSymbol::from_payload()");
  auto source_block_number = 0;
  auto encoding_symbol_id = 0;
  std::vector<EncodingSymbol> symbols;

  if (data_len < 4) {
    spdlog::warn("EncodingSymbol::from_payload() called with invalid data_len {}", data_len);
    return symbols;
  }
...
{
  ZoneScopedN("EncodingSymbol::to_payload()");
  size_t len = 0;
...
auto LibFlute::EncodingSymbol::decode_to(char* buffer, size_t max_length) const -> bool {
  ZoneScopedN("EncodingSymbol::decode_to()");
  switch (_fec_scheme) {
    case FecScheme::CompactNoCode:
    case FecScheme::Raptor:
      // Copy the data to the buffer,
      // we copy max_length bytes or _data_len bytes, whichever is smaller
      if (_data_len <= max_length) {
        memcpy(buffer, _encoded_data, _data_len);
        return true;
      } else { // Being able to get a certain amount of data is useful for debugging. For example, we could print the first bytes of this symbol.However, it should never happen in production
        memcpy(buffer, _encoded_data, max_length);
      }
      break;
    default:
...
auto LibFlute::EncodingSymbol::encode_to(char* buffer, size_t max_length) const -> size_t {
  ZoneScopedN("EncodingSymbol::encode_to()");
  switch (_fec_scheme) {
    default:
    case FecScheme::CompactNoCode:
    case FecScheme::Raptor:
